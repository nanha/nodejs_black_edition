...
static Handle<Value> base64_encode_binding(const Arguments &args)
{
    HandleScope scope;
    char * str;
...
static Handle<Value> base64_decode_binding(const Arguments &args)
{
    HandleScope scope;
...
void Initialize(v8::Handle<v8::Object> target) {
  ...
  NODE_SET_METHOD(target, "encode", base64_encode_binding);
  NODE_SET_METHOD(target, "decode", base64_decode_binding);
}


}  // namespace node

NODE_MODULE(node_base64, node::Initialize)
